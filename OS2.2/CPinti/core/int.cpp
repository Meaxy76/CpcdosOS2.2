...
namespace cpinti
{
	...
	{ 
		...
		bool initialise_interception_exception()
		{
			...
			//						 "Error interception ...",
			//			"core::task_manager", "initialiser_Multitache()",
		...
		void Error_Finalise()
		{
			...
			task_manager::end_SectionCritique();
		...
		void Erreur_Fatale(long signalnum, long ligne, char* fichier, char* fonction)
		{
			...
			Etat_section_critique = task_manager::state_SectionCritique();
			task_manager::begin_SectionCritique();
			...
			printf(" * Current Thread ID : %ld\n", cpinti::task_manager::get_ThreadEnCours());
			printf(" * Total Thread(s)   : %ld\n", cpinti::task_manager::get_NombreThreads());
